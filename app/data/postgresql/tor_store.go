...
func (db *TorStore) Create(ip string, prefix byte, score int) (*models.Tor, error) {
	now := time.Now()
	...
	result, err := sqlx.NamedExec(db,
		"INSERT OR IGNORE INTO tor (ip, prefix, score, created_at, updated_at) VALUES (:ip, :prefix, :score, :created_at, :updated_at)",
		tor,
	)
	if err != nil {
		return nil, err
