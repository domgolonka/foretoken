...
func (c *TxtDomains) Load(body []byte) ([]models.FreeEmail, error) {

	// don't need to update this more than once a day!
	if time.Now().Unix() >= c.lastUpdate.Unix()+(82800) {
		c.iplist = make([]models.FreeEmail, 0)
	}

	f := entity.Feed{
		Logger: c.logger,
	}
	feed, err := f.ReadFile("email_free.json")
	if err != nil {
		return nil, err
	}
...
func (c *TxtDomains) List() ([]models.FreeEmail, error) {
	return c.Load(nil)
}
